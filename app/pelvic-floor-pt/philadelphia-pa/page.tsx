...
export const metadata: Metadata = {
    ...
    description: 'Find pelvic floor physical therapists in Philadelphia, PA. Penn Medicine, Jefferson Health, and CHOP drive strong demand for pelvic floor PT across the Philadelphia metro.',
      ...
      <div className="py-10 px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="flex flex-wrap gap-2 mb-10">
            {['Philadelphia', 'Center City', 'South Philly', 'Manayunk', 'Chestnut Hill', 'Cherry Hill NJ', 'Conshohocken', 'King of Prussia', 'Lansdowne'].map((area) => (<span key={area} className="px-3 py-1.5 bg-stone-100 rounded-full text-xs font-medium text-stone-600">
                {area}
              </span>))}
          </div>
          ...
          {listings.length > 0 ? (<>
              <div className="mb-6 flex items-center justify-between">
                ...
                <Link href={`/listings?location=${encodeURIComponent('Philadelphia, PA')}`} className="flex items-center gap-1.5 text-sm text-teal hover:text-teal-400">
              ...
              <div className="mt-8 text-center">
                <Link href={`/listings?location=${encodeURIComponent('Philadelphia, PA')}`} className="btn-primary">
          ...
          <section className="mt-16">
            ...
            <div className="space-y-5 max-w-3xl">
              {FAQ.map(({ q, a }) => (<div key={q} className="card p-6">
          ...
          <section className="mt-16 rounded-2xl bg-teal p-10 text-center">
            ...
            <p className="text-teal-100 mb-6 max-w-xl mx-auto"> Claim and manage your free listing. </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link href="/submit" className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-8 py-3 text-sm font-semibold text-teal hover:bg-cream-100 transition-colors">
                Add Your Listing Free
              </Link>
              <Link href="/listings?state=Pennsylvania" className="inline-flex items-center justify-center gap-2 rounded-full border-2 border-white/50 px-8 py-3 text-sm font-semibold text-white hover:border-white transition-colors">
