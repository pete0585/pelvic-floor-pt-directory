...
export const metadata: Metadata = {
    ...
    description: 'Find pelvic floor physical therapists in Denver and the Front Range — Aurora, Lakewood, Westminster, Centennial, Englewood, and Littleton. Browse by condition, insurance, and telehealth.',
      ...
      <div className="py-10 px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="flex flex-wrap gap-2 mb-10">
            {['Denver', 'Aurora', 'Lakewood', 'Westminster', 'Centennial', 'Englewood', 'Littleton', 'Arvada', 'Highlands Ranch'].map((area) => (<span key={area} className="px-3 py-1.5 bg-stone-100 rounded-full text-xs font-medium text-stone-600">
                {area}
              </span>))}
          </div>
          ...
          {listings.length > 0 ? (<>
              <div className="mb-6 flex items-center justify-between">
                ...
                <Link href={`/listings?location=${encodeURIComponent('Denver, CO')}`} className="flex items-center gap-1.5 text-sm text-teal hover:text-teal-400">
              ...
              <div className="mt-8 text-center">
                <Link href={`/listings?location=${encodeURIComponent('Denver, CO')}`} className="btn-primary">
          ...
          <section className="mt-16 rounded-2xl bg-teal p-10 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <Link href="/listings?state=Colorado" className="inline-flex items-center justify-center gap-2 rounded-full border-2 border-white/50 px-8 py-3 text-sm font-semibold text-white hover:border-white transition-colors">
