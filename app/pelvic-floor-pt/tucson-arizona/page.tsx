...
export default async function CityPage() {
  ...
  const faqLd = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: "How many pelvic floor pt are in Tucson, Arizona?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Pelvic Floor PT Directory lists 20+ pelvic floor pt in Tucson, Arizona. Counts change as new listings are seeded.",
        },
      },
      {
        "@type": "Question",
        name: "How do I find pelvic floor pt in Tucson?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Search pelvicfloordirectory.com and filter by Tucson. Compare listed providers, then contact the one that fits.",
        },
      },
    ],
  }
  return (
    <>
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        <p className="text-sm text-neutral-500">Tucson, Arizona</p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight">
          Pelvic Floor PT in Tucson, Arizona
        </h1>
        <p className="mt-3 max-w-2xl text-neutral-600">
          20+ listed pelvic floor pt in the Tucson area. Pages are generated from live directory listings — not outreach.
        </p>
          ...
          {listings.map((row: Record<string, unknown>, i: number) => (
            <li key={String(row.id || row.slug || i)} className="rounded-xl border border-neutral-200 p-4">
              ...
              <p className="mt-1 text-sm text-neutral-500">
                {String(row.city || "Tucson")}, {String(row.state || "Arizona")}
              </p>
            </li>
          ))}
        </ul>
