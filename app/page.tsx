...
export const metadata: Metadata = {
    ...
    description: 'Find a pelvic floor PT who specializes in postpartum recovery, incontinence, prolapse, pelvic pain, and more. Search by condition and location.',
};
const CONDITIONS = [
...
export default async function HomePage() {
    ...
    return (<div>
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-cream-100">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {CONDITIONS.map((c) => (<Link key={c.slug} href={`/conditions/${c.slug}`} className="flex flex-col items-center gap-2 rounded-2xl bg-white p-4 text-center shadow-soft hover:shadow-card transition-shadow group">
      ...
      {/* Featured listings */}
      {featured.length > 0 && (<section className="py-16 px-4 sm:px-6 lg:px-8 bg-white">
          <div className="mx-auto max-w-7xl">
            <div className="flex items-center justify-between mb-8">
              ...
              <Link href="/listings" className="hidden sm:flex items-center gap-1.5 text-sm font-semibold text-teal hover:text-teal-400">
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-cream-100">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {TOP_CITIES.map((city) => (<Link key={`${city.name}-${city.state}`} href={`/listings?location=${encodeURIComponent(city.name + ', ' + city.state)}`} className="rounded-xl bg-white px-3 py-3 text-center shadow-soft hover:shadow-card transition-shadow group">
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-teal">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="text-teal-100 text-lg mb-8 leading-relaxed"> Listings are free. Public contact details are available without payment. </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/submit" className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-8 py-4 text-base font-semibold text-teal hover:bg-cream-100 transition-colors">
              Get Listed Free
            </Link>
